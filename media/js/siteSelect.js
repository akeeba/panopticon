...
const elSearch = document.getElementById('siteSelectSearch');
const elSearchContainer = document.getElementById('siteSelectContainer');
const elSearchResults = document.getElementById('siteSelectResultsContainer');
//const alertElement = document.getElementById('siteSelectEmpty');

const elCards = [].slice.call(document.querySelectorAll('.siteSelectServer'));
...
if (elSearch && elSearchContainer) {
  // Add the keyboard event listener which performs the live search in the cards
    ...
    if (typeof (sessionStorage) !== 'undefined') {
      sessionStorage.setItem('panopticon.site.new.search', partialSearch);
    }

    // Iterate all the site cards
    elCards.forEach((card) => {
      // First remove the class which hides the site cards
      ...
      const cardHeader = card.querySelector('.siteTitle');
      const title = cardHeader ? cardHeader.textContent : '';

      // If the site title does not match add a class to hide it.
...
//      alertElement.classList.remove('d-none');
  ...
  try {
    if (typeof (sessionStorage) !== 'undefined') {
      ...
      elSearch.value = sessionStorage.getItem('panopticon.site.new.search') || '';
