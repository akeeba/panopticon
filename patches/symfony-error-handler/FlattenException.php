 ...
 * ---------------------------------------------------------------------------------------------------------------------
 *
 * This is a verbatim copy of Symfony\Component\ErrorHandler\Exception\FlattenException from symfony/error-handler
 * v6.4.36, with a single deliberate modification (see the `AKEEBA PANOPTICON CUSTOMISATION` marker below).
 *
 * It is loaded, in place of the upstream class, by Akeeba\Panopticon\Application\BootstrapUtilities::
 * overrideHtmlErrorRenderer() BEFORE Composer's autoloader can define the upstream version. We used to perform this
 * modification at runtime (read upstream source, str_replace, write to a stream wrapper, require, unlink). That
 ...
 * file hash changed. Re-copy the upstream class here, re-apply the marked modification, and update the hash in that
...
class FlattenException
{
    ...
    public static function createFromThrowable(\Throwable $exception, ?int $statusCode = null, array $headers = []): static
    {
        ...
        if (class_exists(Response::class) && isset(Response::$statusTexts[$statusCode])) {
            ...
            // Upstream sets a generic 'Whoops, looks like something went wrong.' here. We surface the actual
            // exception message instead so it is available to our custom fatal-error template.
