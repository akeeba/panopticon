...
/*
 * ---------------------------------------------------------------------------------------------------------------------
 * AKEEBA PANOPTICON — VENDORED, LOCALLY MODIFIED COPY
 * ---------------------------------------------------------------------------------------------------------------------
 *
 ...
 *      relies on the `$exception` variable, which upstream does NOT pass to the simple page.
 *   2. include(): template/asset resolution is made overridable from the active theme (templates/system, then
 *      error/...) before falling back to Symfony's bundled Resources.
 *
 * It is loaded, in place of the upstream class, by Akeeba\Panopticon\Application\BootstrapUtilities::
 * overrideHtmlErrorRenderer() BEFORE Composer's autoloader can define the upstream version. We used to perform these
 * modifications at runtime (read upstream source, str_replace, write to a stream wrapper, require, unlink). That
 * self-modifying pattern is indistinguishable from a malware "dropper" to heuristic scanners such as Imunify360, so we
 * now ship the modified class as an ordinary, auditable file instead.
 *
...
class HtmlErrorRenderer implements ErrorRendererInterface
{
    ...
    private function renderException(FlattenException $exception, string $debugTemplate = 'views/exception_full.html.php'): string
    {
        ...
        if (!$debug) {
            ...
            // Upstream passes only statusText/statusCode to the non-debug ("simple") template. We pass the same rich
            // context as the debug template below, because our custom fatal-error template (templates/system/fatal.php)
            // uses the `$exception` variable directly (getMessage()/getCode()/getFile()/getLine()/getTraceAsString()).
            ...
            // --- END AKEEBA PANOPTICON CUSTOMISATION ------------------------------------------------------------------
        }
    ...
    private function include(string $name, array $context = []): string
    {
        ...
        // --- AKEEBA PANOPTICON CUSTOMISATION -------------------------------------------------------------------------
        // Upstream resolves templates/assets only against Symfony's bundled Resources directory. We first allow the
        // active theme to override them (templates/system/..., with `views/` mapped to `error/`), then fall back to
        // Symfony's bundled Resources, then to the raw name.
