...
/**
 * @var \Akeeba\Panopticon\View\Extensions\Html $this
 * @var \Akeeba\Panopticon\Model\Extensions     $model
...
<form action="@route('index.php?view=extensions')" method="post" name="adminForm" id="adminForm">
    <!-- Filters -->
    <div class="my-2 border rounded-1 p-2 bg-body-tertiary">
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-lg-center gap-2 mt-2">
            <!-- Site -->
            <div>
                <label class="visually-hidden" for="site_id">@lang('PANOPTICON_EXTENSIONS_LBL_SITE')</label>
                {{  $this->container->html->select->genericList(
                        $mainModel->getSiteNamesForSelect(true, 'PANOPTICON_EXTENSIONS_LBL_SITE_SELECT'),
                        'site_id',
                        [
            ...
            <div>
                ...
                {{ $this->container->html->select->genericList(
                    array_merge([
                        '' => $this->getLanguage()->text('PANOPTICON_EXTENSIONS_LBL_CMSVERSION_SELECT')
            ...
            <div>
                ...
                {{ $this->container->html->select->genericList(
                    array_merge([
                        '' => $this->getLanguage()->text('PANOPTICON_EXTENSIONS_LBL_PHPVERSION_SELECT')
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-lg-center gap-2 mt-2">
            <div>
                <label class="visually-hidden" for="extension_name">@lang('PANOPTICON_EXTENSIONS_LBL_EXT_NAME')</label>
                {{  $this->container->html->select->genericList(
            ...
            <div>
                <label class="visually-hidden"
                       for="extension_author">@lang('PANOPTICON_EXTENSIONS_LBL_EXT_AUTHOR')</label>
                {{  $this->container->html->select->genericList(
            ...
            <div>
                <label class="visually-hidden"
                       for="extension_author_url">@lang('PANOPTICON_EXTENSIONS_LBL_EXT_AUTHOR_URL')</label>
                {{  $this->container->html->select->genericList(
    ...
    <table class="table table-striped align-middle pnp-stacked" id="adminList" role="table">
        <caption class="visually-hidden">
            @lang('PANOPTICON_EXTENSIONS_TABLE_COMMENT')
        </caption>
        <thead>
        <tr>
            <th>
                {{ $this->getContainer()->html->grid->sort('PANOPTICON_EXTENSIONS_FIELD_SITE', 'id', $this->lists->order_Dir, $this->lists->order, 'main') }}
            </th>
            <th>
                @lang('PANOPTICON_EXTENSIONS_FIELD_EXTENSION')
            </th>
            <th>
                @lang('PANOPTICON_EXTENSIONS_FIELD_VERSION')
            </th>
            <th>
                @lang('PANOPTICON_EXTENSIONS_FIELD_AUTHOR')
            </th>
        </tr>
        </thead>
        <tbody>
        @foreach($this->items as $i => $item)
				...
				?>
            <tr>
                <td data-label="@lang('PANOPTICON_EXTENSIONS_FIELD_SITE')">
                    <a class="fw-medium"
                ...
                <td data-label="@lang('PANOPTICON_EXTENSIONS_FIELD_EXTENSION')">
                    <div>
                    ...
                    @if ($missingDownloadID)
                        ...
                        @if ($this->container->userManager->getUser()->authorise('panopticon.admin', $site->id))
                            <a href="@route(sprintf('index.php?view=site&task=dlkey&id=%d&extension=%d&%s=1', $site->id, $extIdForUpdate, $this->container->session->getCsrfToken()->getValue()))"
                ...
                <td data-label="@lang('PANOPTICON_EXTENSIONS_FIELD_VERSION')">
                    @if ($hasUpdate && $error)
                ...
                <td class="small" data-label="@lang('PANOPTICON_EXTENSIONS_FIELD_AUTHOR')">
                    <div>
                        @if ($item->authorUrl)
