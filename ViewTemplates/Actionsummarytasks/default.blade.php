...
/**
 * @var \Akeeba\Panopticon\View\Actionsummarytasks\Html $this
...
<form action="@route('index.php?view=actionsummarytasks')" method="post" name="adminForm" id="adminForm">
    ...
    <div class="alert alert-info">
        ...
        @sprintf(
          'PANOPTICON_ACTIONSUMMARYTASKS_LBL_TIMEZONE_NOTICE',
    ...
    <table class="table table-striped align-middle" id="adminList" role="table">
        ...
        @foreach($this->items as $task)
				...
				$period = ActionReportPeriod::tryFrom($params->get('period')) ?: ActionReportPeriod::DAILY;
				?>
            <tr>
                {{-- Checkbox --}}
                <td>
                ...
                <td>
                    <div>
                        <a href="@route(sprintf('index.php?view=actionsummarytasks&task=edit&site_id=%d&id=%d', $this->site->id, $task->id))">
                    ...
                    <div class="mt-1 pt-1 border-top d-flex flex-row gap-0">
                        ...
                        <span class="fa fa-fw fa-stack fa-calendar-alt" aria-hidden="true"
                              data-bs-toggle="tooltip" data-bs-placement="bottom"
                              data-bs-title="@lang('PANOPTICON_ACTIONSUMMARYTASKS_LBL_PERIOD')"></span>
                        <span class="visually-hidden">@lang('PANOPTICON_ACTIONSUMMARYTASKS_LBL_PERIOD')</span>
                        <span class="fa-stack">
                            @lang('PANOPTICON_ACTIONSUMMARYTASKS_LBL_PERIOD_' . $period->value)
                        </span>
                    </div>
                </td>
                {{-- Enabled --}}
                <td>
                    @if ($task->enabled)
                        <a class="text-decoration-none text-success"
                           href="@route(sprintf('index.php?view=actionsummarytasks&site_id=%d&task=unpublish&id=%d&%s=1', $this->site->id, $task->id, $token))"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
                    ...
                    @else
                        <a class="text-decoration-none text-danger"
                           href="@route(sprintf('index.php?view=actionsummarytasks&task=publish&site_id=%d&id=%d&%s=1', $this->site->id, $task->id, $token))"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
