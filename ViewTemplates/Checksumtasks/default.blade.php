...
/**
 * @var \Akeeba\Panopticon\View\Checksumtasks\Html $this
...
<form action="@route('index.php?view=checksumtasks')" method="post" name="adminForm" id="adminForm">
    <div class="my-2 border rounded-1 p-2 bg-body-tertiary">
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-lg-center gap-2 mt-2">
            ...
            <div>
                <label class="visually-hidden" for="manual">@lang('PANOPTICON_CHECKSUMTASKS_LBL_VIEW_MANUAL')</label>
                {{ $this->container->html->select->genericList([
	                '' => 'PANOPTICON_CHECKSUMTASKS_LBL_SELECT_MANUAL',
	                '0' => 'PANOPTICON_CHECKSUMTASKS_LBL_MANUAL_NO',
	                '1' => 'PANOPTICON_CHECKSUMTASKS_LBL_MANUAL_YES',
                ], 'manual', [
					'class' => 'form-select akeebaGridViewAutoSubmitOnChange',
    ...
    <div class="alert alert-info">
        ...
        @sprintf(
          'PANOPTICON_CHECKSUMTASKS_LBL_TIMEZONE_NOTICE',
    ...
    <table class="table table-striped align-middle" id="adminList" role="table">
        <caption class="visually-hidden">
            @lang('PANOPTICON_CHECKSUMTASKS_TABLE_COMMENT')
        </caption>
        <thead>
        <tr>
            <th class="pnp-w-1">
        ...
        @foreach($this->items as $task)
            ...
            <tr>
                ...
                {{-- Schedule --}}
                <td>
                    @if($params->get('enqueued_checksums'))
                        <div class="d-flex flex-column flex-lg-row align-items-center gap-1 text-body-tertiary">
                            ...
                            <span>@lang('PANOPTICON_CHECKSUMTASKS_LBL_MANUAL_SCAN')</span>
                        </div>
                    @else
                        <div>
                            <a href="@route(sprintf('index.php?view=checksumtask&task=edit&site_id=%d&id=%d', $this->site->id, $task->id))">
                        ...
                        <div class="mt-1 pt-1 border-top">
                            @if($params->get('run_once') == 'disable')
                                <span class="fa fa-circle-stop me-2 text-warning" aria-hidden="true"
                                      ...
                                      data-bs-title="@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_DISABLE')"></span>
                                <span class="visually-hidden">@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_DISABLE')</span>
                            @elseif($params->get('run_once') == 'delete')
                                <span class="fa fa-land-mine-on me-2 text-danger" aria-hidden="true"
                                      data-bs-toggle="tooltip" data-bs-placement="bottom"
                                      data-bs-title="@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_DELETE')"></span>
                                <span class="visually-hidden">@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_DELETE')</span>
                            @else
                                <span class="fa fa-repeat me-2 text-success-emphasis" aria-hidden="true"
                                      data-bs-toggle="tooltip" data-bs-placement="bottom"
                                      data-bs-title="@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_NONE')"></span>
                                <span class="visually-hidden">@lang('PANOPTICON_CHECKSUMTASKS_LBL_FIELD_RUN_ONCE_NONE')</span>
                            @endif
                        </div>
                    @endif
                </td>
                ...
                <td>
                    ...
                    @elseif ($task->enabled)
                        <a class="text-decoration-none text-success"
                           href="@route(sprintf('index.php?view=checksumtasks&site_id=%d&task=unpublish&id=%d&%s=1', $this->site->id, $task->id, $token))"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
                    ...
                    @else
                        <a class="text-decoration-none text-danger"
                           href="@route(sprintf('index.php?view=checksumtasks&task=publish&site_id=%d&id=%d&%s=1', $this->site->id, $task->id, $token))"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
