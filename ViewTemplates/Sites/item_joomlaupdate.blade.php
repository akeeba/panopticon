    ...
    <div class="card-body">
        ...
        @elseif(!$this->siteConfig->get('core.updateSiteAvailable', true))
            <div class="alert alert-danger">
                ...
                @if(version_compare($this->siteConfig->get('core.current.version', '4.0.0'), '4.0.0', 'ge'))
                    @lang('PANOPTICON_SITE_LBL_JUPDATE_UPDATESITE_FIX_J4')
                @elseif(version_compare($this->siteConfig->get('core.current.version', '4.0.0'), '3.6.0', 'ge'))
        ...
        @elseif ($this->siteConfig->get('core.canUpgrade', false))
            <div class="alert alert-warning">
                <h4 class="alert alert-heading h5 p-0">
                    <span class="fab fa-joomla d-none d-md-inline" aria-hidden="true"></span>
                    @sprintf('PANOPTICON_SITE_LBL_JUPDATE_AVAILABLE_UPDATE', $this->escape($this->siteConfig->get('core.latest.version')))
                </h4>
                <p class="mb-1">
                    @sprintf('PANOPTICON_SITE_LBL_JUPDATE_CURRENT_VERSION', $this->escape($this->siteConfig->get('core.current.version')))
                </p>
					<?php
					$versionCurrent = Version::create($this->siteConfig->get('core.current.version'));
					$versionLatest  = Version::create($this->siteConfig->get('core.latest.version'));
					?>
        ...
        @elseif($jVersionHelper->isEOLMajor($this->siteConfig->get('core.current.version')))
            <?php
            $versionCurrent    = Version::create($this->siteConfig->get('core.current.version'));
            $versionFamilyInfo = $jVersionHelper->getVersionInformation($this->siteConfig->get('core.current.version'));
            ...
            <div class="alert alert-danger text-danger-emphasis">
                <h4 class="alert alert-heading h5 p-0 m-0">
                    @sprintf('PANOPTICON_SITE_LBL_JUPDATE_EOL', $this->escape($this->siteConfig->get('core.latest.version')))
                </h4>
                ...
                @if (version_compare($this->siteConfig->get('core.latest.version'), $this->siteConfig->get('core.current.version'), 'lt'))
                    <hr>
                    <p class="my-2 text-warning-emphasis fw-semibold">
                        @sprintf('PANOPTICON_SITE_LBL_JUPDATE_CANNOT_INSTALL', $this->escape($this->siteConfig->get('core.latest.version')))
                    </p>
                    <p class="text-muted">
        ...
        @elseif($jVersionHelper->isEOLBranch($this->siteConfig->get('core.current.version')))
            <?php
            $versionCurrent = Version::create($this->siteConfig->get('core.current.version'));
            ?>
            <div class="alert alert-danger text-danger-emphasis">
        ...
        @else
            ...
            <div class="alert alert-success">
                <h4 class="alert alert-heading h5 p-0 m-0">
                    @sprintf('PANOPTICON_SITE_LBL_JUPDATE_UP_TO_DATE', $this->escape($this->siteConfig->get('core.current.version')))
                </h4>
                {{-- Is there a new version available, which cannot be installed? --}}
                @if (version_compare($this->siteConfig->get('core.latest.version'), $this->siteConfig->get('core.current.version'), 'lt'))
                    <hr>
        ...
        @if ($showScheduleButton)
            <a href="@route(sprintf('index.php?view=site&task=scheduleJoomlaUpdate&id=%d&%s=1', $this->item->id, $token))"
               class="btn btn-outline-warning" role="button">
                ...
                @sprintf('PANOPTICON_SITE_LBL_JUPDATE_SCHEDULE_UPDATE', $this->escape($this->siteConfig->get('core.latest.version')))
            </a>
                    ...
                    <p>
                        <a href="@route(sprintf('index.php?view=site&task=scheduleJoomlaUpdate&id=%d&force=1&%s=1', $this->item->id, $token))"
                           class="btn btn-outline-secondary" role="button">
                            <span class="fa fa-clock" aria-hidden="true"></span>
                            @sprintf('PANOPTICON_SITE_LBL_JUPDATE_BTN_REFRESH_CORE_PROMPT', $this->escape($this->siteConfig->get('core.latest.version')))
                        </a>
                    </p>
                    @endif
