...
trait EnqueuePluginUpdateTrait
{
	private function schedulePluginsUpdateForSite(Site $site, Container $container, bool $force = false): void
	{
		/** @var Task $task */
	...
	private function enqueuePluginUpdate(
		Site $site, string $softwareId, string $effectivePreference = 'major', ?User $user = null
	): bool
	{
		// Enqueue necessary updates
		$queueKey = sprintf(QueueTypeEnum::PLUGINS->value, $site->id);
		$queue    = $this->container->queueFactory->makeQueue($queueKey);

		// Avoid enqueueing the same extension multiple times
		$numItems = $queue->countByCondition(['data.id' => $softwareId, 'siteId' => (int) $site->id]);
