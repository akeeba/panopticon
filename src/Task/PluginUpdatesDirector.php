...
#[AsTask(
	name: 'pluginupdatesdirector',
	description: 'PANOPTICON_TASKTYPE_PLUGINUPDATESDIRECTOR'
)]
class PluginUpdatesDirector extends AbstractCallback
{
	...
	public function __invoke(object $task, Registry $storage): int
	{
		...
		if (empty($siteIDs))
		{
			$this->logger->info('No more sites in need of automatic plugin / theme updates.');
		...
		$this->logger->info(sprintf(
			'Found a further %d site(s) to process for automatic plugin / theme updates.',
			count($siteIDs)
		));
		...
		foreach ($siteIDs as $id)
		{
			...
			if (empty($extensions))
			{
				$this->logger->debug(
					sprintf(
						'There are no known plugins / themes for site #%d (%s)',
						$site->id, $site->name
					)
				);
			...
			$extensions = array_filter(
				...
				{
					/**
					 * Can't update a plugin / theme without version information, or when the installed version is newer
					...
					// Get the plugin / theme shortname (key)
					$key = $sysConfigModel
					...
					// Exclude plugins / themes we are told to do nothing with.
			...
			if (empty($extensions))
			{
				$this->logger->debug(
					sprintf(
						'There are no plugins / themes which need to be updated on site #%d (%s)',
						$site->id, $site->name
					)
				);
			...
			foreach ($extensions as $item)
			{
				...
				if (empty($key))
				{
					// Invalid plugin / theme
					continue;
				}

				// Log plugin / theme update found
				try {
				...
				/**
				 * Plugins / themes here can have an effective preference of email, major, minor, or patch.
				 *
			...
			if ($numExtensionsUpdate > 0)
			{
				if ($numExtensionsUpdate === 1)
				{
					$this->logger->debug(
						sprintf(
							'1 plugin / theme was queued for automatic updates on site #%d (%s)',
							$site->id, $site->name
						)
					);
				}
				else
				{
					$this->logger->debug(
						sprintf(
							'%d plugins / themes were queued for automatic updates on site #%d (%s)',
			...
			if ($numExtensionsEmail > 0)
			{
				if ($numExtensionsEmail === 1)
				{
					$this->logger->debug(
						sprintf(
							'1 plugin / theme was queued for update email on site #%d (%s)',
							$site->id, $site->name
						)
					);
				}
				else
				{
					$this->logger->debug(
						sprintf(
							'%d plugins / themes were queued for update emails on site #%d (%s)',
							$numExtensionsUpdate, $site->id, $site->name
						)
					);
				}
			...
			if ($numExtensionsUpdate <= 0 && $numExtensionsEmail <= 0)
			{
				$this->logger->debug(
					sprintf(
						'No plugins / themes were queued for automatic updates or email on site #%d (%s) - all plugins / themes were already queued up',
						$site->id, $site->name
					)
				);
			}
