...
class Setup extends Model
{
	...
	public function installDefaultGroups(): void
	{
		$db    = $this->container->db;
		$query = $db->getQuery(true)
			->select('COUNT(*)')
			->from($db->quoteName('#__groups'))
			->where($db->quoteName('title') . ' = ' . $db->quote('Users'));
		...
		$db->insertObject('#__groups', $o, 'id');
	}

	public function installDefaultMailTemplates(): void
	{
		$db = $this->container->db;

		$defaults = [
			'registration_pending_admin' => [
				'subject'   => '[SITENAME] Your registration is pending approval',
				'html'      => '<p>Hello [NAME],</p><p>Your account on <strong>[SITENAME]</strong> has been created and is pending administrator approval.</p><p>Username: <strong>[USERNAME]</strong></p><p>You will receive another email once your account has been approved.</p>',
			],
			'registration_activate' => [
				'subject'   => '[SITENAME] Activate your account',
				'html'      => '<p>Hello [NAME],</p><p>Your account on <strong>[SITENAME]</strong> has been created. Please activate your account by clicking the link below:</p><p><a href="[ACTIVATION_URL]">[ACTIVATION_URL]</a></p><p>Username: <strong>[USERNAME]</strong></p><p>You have <strong>[EXPIRY_DAYS] days</strong> to activate your account before it is automatically deleted.</p><p>If the link does not work, visit the activation page manually and enter:<br>Token: <strong>[TOKEN]</strong></p>',
			],
			'registration_approved' => [
				'subject'   => '[SITENAME] Your account has been approved',
				'html'      => '<p>Hello [NAME],</p><p>Your account on <strong>[SITENAME]</strong> has been approved. You can now log in at:</p><p><a href="[LOGIN_URL]">[LOGIN_URL]</a></p><p>Username: <strong>[USERNAME]</strong></p>',
			],
			'registration_expired' => [
				'subject'   => '[SITENAME] Your registration has expired',
				'html'      => '<p>Hello [NAME],</p><p>Your registration on <strong>[SITENAME]</strong> has expired because it was not activated in time, or too many failed activation attempts were made.</p><p>You are welcome to register again.</p>',
			],
		];

		foreach ($defaults as $type => $template)
		{
			// Only insert if no template of this type exists
			$query = $db->getQuery(true)
				->select('COUNT(*)')
			...
			$db->insertObject('#__mailtemplates', $o, 'id');
		}
	}
