...
#[AsCommand(
	name: 'mailtemplate:set:plaintext',
	description: 'Set the plaintext part of an existing mail template',
	hidden: false,
)]
#[ConfigAssertion(true)]
class MailtemplateSetPlaintext extends AbstractCommand
{
	protected function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		{
			// Read from stdin
			$plaintext = file_get_contents('php://stdin');
		}
		...
		$model->bind(['plaintext' => $plaintext]);
		...
		$this->ioStyle->success(sprintf('Successfully updated plaintext for mail template "%s"', $id));
	...
	protected function configure(): void
	{
		$this
			...
			->addOption('plaintext', 'p', InputOption::VALUE_REQUIRED, 'The plaintext body of the mail template. If omitted, it will be read from stdin.');
	}
