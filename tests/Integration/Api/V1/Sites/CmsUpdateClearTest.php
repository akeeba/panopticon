...
use Akeeba\Panopticon\Controller\Api\V1\Site\CmsUpdateClear;
use Akeeba\Panopticon\Model\Site;
use Akeeba\Panopticon\Tests\Integration\Api\AbstractApiIntegrationTestCase;

/**
 * Integration tests for POST /v1/site/:id/cmsupdate/clear.
 *
 * @since  1.4.0
 */
class CmsUpdateClearTest extends AbstractApiIntegrationTestCase
{
	public function testUnauthorisedWithNoToken(): void
	{
		$response = $this->dispatchApi('V1\\Site\\CmsUpdateClear', ['id' => 1]);
	...
	public function testNotFoundForUnknownId(): void
	{
		...
		$response = $this->invokeHandler(CmsUpdateClear::class, ['id' => 999999999]);
	...
	public function testForbiddenWithoutRunPrivilege(): void
	{
		...
		$site->save([
			'name'    => 'CmsClear ACL site',
			'url'     => 'https://cmsclear-acl.test/api',
			'enabled' => 1,
		...
		$response = $this->invokeHandler(CmsUpdateClear::class, ['id' => (int) $site->getId()]);
	...
	public function testNoTaskReturns404(): void
	{
		...
		$site = $this->container->mvcFactory->makeTempModel('Site');
		$site->save([
			'name'    => 'CmsClear no task',
			'url'     => 'https://cmsclear-none.test/api',
			'enabled' => 1,
			'config'  => json_encode(['cmsType' => 'joomla']),
		]);
