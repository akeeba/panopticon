...
use Akeeba\Panopticon\Controller\Api\V1\Site\ExtensionScheduleUpdate;
use Akeeba\Panopticon\Model\Site;
use Akeeba\Panopticon\Tests\Integration\Api\AbstractApiIntegrationTestCase;

/**
 * Integration tests for POST /v1/site/:id/extensions/scheduleupdate/:extId.
 *
 * @since  1.4.0
 */
class ExtensionScheduleUpdateTest extends AbstractApiIntegrationTestCase
{
	public function testUnauthorisedWithNoToken(): void
	{
		$response = $this->dispatchApi('V1\\Site\\ExtensionScheduleUpdate', ['id' => 1, 'extId' => 42]);
	...
	public function testNotFoundForUnknownId(): void
	{
		...
		$response = $this->invokeHandler(ExtensionScheduleUpdate::class, ['id' => 999999999, 'extId' => 42]);
	...
	public function testForbiddenWithoutRun(): void
	{
		...
		$site->save([
			'name'    => 'ExtSched ACL site',
			'url'     => 'https://extsched-acl.test/api',
			'enabled' => 1,
			'config'  => json_encode([
				'cmsType'    => 'joomla',
	...
	public function testBadRequestForZeroExtId(): void
	{
		...
		$site = $this->container->mvcFactory->makeTempModel('Site');
		$site->save([
			'name'    => 'ExtSched bad extId',
			'url'     => 'https://extsched-bad.test/api',
			'enabled' => 1,
	...
	public function testExtensionNotFoundReturns404(): void
	{
		...
		$site = $this->container->mvcFactory->makeTempModel('Site');
		$site->save([
			'name'    => 'ExtSched no-ext',
			'url'     => 'https://extsched-noext.test/api',
			'enabled' => 1,
			'config'  => json_encode([
				'cmsType'    => 'joomla',
				'extensions' => ['list' => []],
